...
const New = ({postProcess}) => {
    ...
    let [kapanId, cutId, process] = ids.split("-");
    if(postProcess){
        kapanId = 1;
    }
    ...
    const handleSubmit = (e) => {
        Cart.addPacket(kapanId, cutId, process, data,postProcess)
            .then(res => {
                if (res.err) {
                    ...
                    navigate(`/${postProcess?"PP":""}cart/${ids}`);
                }
            })
            .catch(err => {
    ...
    const handleChange = (e) => {
        ...
        const updatedValue = { ...data, [name]: value, size: size.toFixed(2) }
